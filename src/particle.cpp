...
void Particle::collide(PointMass &pm) {
...
void Particle::moveTo(Vector3D newPos) {
    this -> origin = newPos;
}

void Particle::moveStep(Vector3D newPos) {
    this -> origin += newPos;
}

void Particle::simulate(double frames_per_sec,
...
//    Vector3D newAccel = force / this->mass;
    Vector3D newPos = currPos + (currPos - collisionPt) * pow(delta_t, 2);
    this->currPosition = newPos;
    this->prevPosition = currPos;
    this->origin = newPos;

//    for (CollisionObject *cp : *collision_objs) {
//        Particle* sq = dynamic_cast<Particle*>(cp);
...
void Particle::collideSphere(Particle &p, bool realistic, Vector3D &collisionPt) {
